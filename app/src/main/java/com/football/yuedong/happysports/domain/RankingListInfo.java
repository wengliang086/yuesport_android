 ...
 */
public class RankingListInfo extends BaseResponse{
    ...
    protected RankingListInfo(Parcel in) {
        super(in);
        this.value = in.createTypedArrayList(ValueEntity.CREATOR);
    }

    public static final Creator<RankingListInfo> CREATOR = new Creator<RankingListInfo>() {
        public RankingListInfo createFromParcel(Parcel source) {
            return new RankingListInfo(source);
        }

        public RankingListInfo[] newArray(int size) {
            return new RankingListInfo[size];
        }
    };
}
